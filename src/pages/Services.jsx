    ...
    <div style={{ background: 'transparent', color: T.ink, fontFamily: F.body, minHeight: '100vh' }}>
      ...
      <section style={{ padding: isMobile ? '100px 20px 20px' : isTablet ? '120px 30px 30px' : '140px 40px 40px', position: 'relative' }}>
        ...
        <div style={{ position: 'relative' }}>
          <SPStatusSticker T={T} F={F} top={isMobile ? -40 : isTablet ? -30 : -20} left={isMobile ? 20 : isTablet ? 140 : 340} rotate={-6} variant="tape" tape={T.popA} bg={mint} icon="$" iconBg={T.ink} label="Starts at" name="₹40k" scale={isMobile ? 0.6 : isTablet ? 0.75 : 1} />
          <SPStatusSticker T={T} F={F} top={isMobile ? 90 : isTablet ? 110 : 140} right={isMobile ? 10 : isTablet ? 40 : 140} rotate={7} variant="torn" bg={butter} icon="⏱" iconBg={T.popB} label="Typical" name="6–12 weeks" scale={isMobile ? 0.6 : isTablet ? 0.75 : 1} />
          <SPStar size={isMobile ? 35 : isTablet ? 45 : 60} color={T.popA} rotate={18} top={isMobile ? 30 : isTablet ? 40 : 60} right={isMobile ? 20 : isTablet ? 40 : 60} />
          ...
          <h1 style={{ fontFamily: F.display, fontSize: 'clamp(60px, 15vw, 260px)', lineHeight: 0.84, fontWeight: 700, letterSpacing: '-0.05em', margin: 0 }}>
            the <span style={{ fontFamily: F.italic, fontStyle: 'italic', fontWeight: 400, color: T.popA }}>menu,</span>
          </h1>
        </div>
      ...
      <section style={{ padding: isMobile ? '20px 20px 60px' : isTablet ? '30px 30px 60px' : '40px 40px 80px' }}>
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(12,1fr)', gridAutoRows: 'auto', gap: 16 }}>
          {menu.map((m, i) => (
            <article key={i} style={{
              gridColumn: isMobile ? '1 / -1' : isTablet ? (m.big ? 'span 12' : 'span 6') : `span ${m.big ? 12 : 6}`,
              ...
              display: 'grid', gridTemplateColumns: isMobile || isTablet ? '1fr' : (m.big ? '1fr 1.4fr' : '1fr'), gap: isMobile || isTablet ? 32 : (m.big ? 48 : 0),
              ...
              <div style={{ display: 'grid', gridTemplateColumns: isMobile || isTablet ? '1fr' : (m.big ? '1fr 1fr' : '1fr'), gap: m.big ? 24 : 16, marginTop: isMobile || isTablet ? 0 : (m.big ? 0 : 24) }}>
                <div>
      ...
      <section style={{ padding: isMobile ? '60px 20px' : isTablet ? '80px 30px' : '100px 40px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : isTablet ? 'repeat(2,1fr)' : 'repeat(5,1fr)', gap: isMobile || isTablet ? 20 : 12 }}>
          {process.map((p, i) => (
            <div key={i} style={{
      ...
      {/* CTA */}
      <section style={{ padding: isMobile ? '80px 20px' : isTablet ? '100px 30px' : '120px 40px', textAlign: 'center', background: butter, borderBottom: `1px solid ${T.ink}`, position: 'relative', overflow: 'hidden' }}>
